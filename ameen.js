...
async function gandu() {
  try {
    ...
    if (gnome && gnome.trim().toLowerCase() !== lund.toLowerCase()) {
      ...
      console.log(`${endi}`)
      console.log(chalk.bgBlack(chalk.redBright('Initializing 𝐊𝐞𝐢𝐤𝐨 𝐕𝟓')))
    }
  } catch (error) {
    console.error('Error:', error)
  }
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    let msgf = `Hey🙊 ${name} Congrats You Have Successfully Deployed 𝐊𝐞𝐢𝐤𝐨 𝐕𝟓\nJoin My Support Group For Any Query\nhttps://chat.whatsapp.com/GVxT4w51GIU3sndNPZGTnw\n> ©Ameen-Ser`
