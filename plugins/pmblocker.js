...
export async function before(m, { conn, isAdmin, isBotAdmin, isOwner, isROwner }) {
    ...
    if (!m.isGroup) {
      ...
      if (bot.pmblocker && !isOwner && !isROwner) {
        ...
        await m.reply(`*Hey @${m.sender.split`@`[0]}, messaging the 🤍 DURGA-MD 🤍─❂  bot privately is currently disabled. You have been blocked from using the 🤍 DURGA-MD 🤍─❂ bot.*`, false, { mentions: [m.sender] });
